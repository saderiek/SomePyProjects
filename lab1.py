...
def main():
    ...
    parser.add_argument('-w', '--write', nargs=2, type=str, help='Writing to a file')
    parser.add_argument('-r', '--read', type=str, help='Reading from a file')
    parser.add_argument('-rn', '--rename', nargs=2, type=str, help='Renaming a file')
    parser.add_argument('-c', '--copy', nargs=2, type=str, help='Copying a file from one directory to another')

    ## REGISTRY ##
    parser.add_argument('-kc', '--kcreate', type=str, help='Creating a key')
    parser.add_argument('-kd', '--kdelete', type=str, help='Deleting a key')
    parser.add_argument('-kv', '--kvalue', type=str, help='Writing a value to a keyy')
    ...
    elif (args.kcreate): create_key(args.kcreate)
    elif (args.kdelete): delete_key(args.kdelete)
    elif (args.kvalue): value_key(args.kvalue)
